...
class SequentialTransformerQNetwork(nn.Module):
    ...
    def forward(self, state_sequence, action_history=None):
        ...
        embedded_states = []
        for i in range(state_sequence.shape[2]):
            # Extract the i-th feature for all sequences and embed it
            feature = state_sequence[:, :, i].unsqueeze(-1)  # [batch_size, seq_len, 1]
            embedded_feature = self.state_embedding(feature)  # [batch_size, seq_len, hidden_dim//2]
            embedded_states.append(embedded_feature)
        
        # Combine all embedded features
        state_embeddings = torch.cat(embedded_states, dim=-1)  # [batch_size, seq_len, hidden_dim]
        
        # Process state embeddings only if no action history
        if action_history is None or seq_len == 1:
            # Just use state embeddings directly
            x = state_embeddings
        else:
            # Embed action history
            action_embeddings = self.action_embedding(action_history)  # [batch_size, seq_len-1, hidden_dim//2]
            
            # Pad action embeddings to match state sequence length
            padded_action_embeddings = torch.zeros(
                batch_size, seq_len, action_embeddings.shape[-1], 
            ...
            combined = torch.cat([state_embeddings, padded_action_embeddings], dim=-1)
...
class SequentialDQNAgent:
    ...
    def get_current_sequences(self):
        ...
        if not self.initialized:
            ...
            # Ensure first state is a numpy array
            if not isinstance(state_seq[0], np.ndarray):
                state_seq[0] = np.array(state_seq[0], dtype=np.float32)
                
            padding = [np.zeros_like(state_seq[0])] * (self.sequence_length - len(state_seq))
        ...
        # Convert all states to numpy arrays of the same shape
        for i in range(len(state_seq)):
            if not isinstance(state_seq[i], np.ndarray):
                state_seq[i] = np.array(state_seq[i], dtype=np.float32)
        ...
        try:
            ...
            # If shapes are inconsistent, handle each state individually
            shapes = [s.shape for s in state_seq]
            print(f"Warning: Inconsistent state shapes in buffer: {shapes}")
            
            # Reshape all states to match the first state's shape
            first_shape = state_seq[0].shape
            for i in range(1, len(state_seq)):
                if state_seq[i].shape != first_shape:
                    state_seq[i] = np.resize(state_seq[i], first_shape)
    ...
    def learn(self, replay_buffer, batch_size):
        ...
        try:    
            ...
            q_expected = q_values.gather(1, actions[:, -1].unsqueeze(1))
...
class SequenceTracker:
    ...
    def get_state_sequence(self):
        ...
        if len(state_seq) < self.sequence_length:
            # Use the first state for padding if available
            padding_state = state_seq[0].copy()
            padding = [padding_state.copy() for _ in range(self.sequence_length - len(state_seq))]
            state_seq = padding + state_seq
        ...
        for i in range(1, len(state_seq)):
            if state_seq[i].shape != first_shape:
                # Resize inconsistent shapes
                state_seq[i] = np.resize(state_seq[i], first_shape)
        
        try:
            # Try stacking the states
            return np.stack(state_seq)
        except ValueError as e:
            # If there's still an issue, print debug info and handle it
            shapes = [s.shape for s in state_seq]
            print(f"Warning: Failed to stack states with shapes: {shapes}")
            print(f"Error: {e}")
            
            # Create a new array with the correct shape and copy data
            result = np.zeros((self.sequence_length,) + first_shape)
            for i, s in enumerate(state_seq):
                # Try to copy compatible portions
                try:
                    result[i, :len(s)] = s[:len(result[i])]  
                except:
                    pass  # Skip incompatible states
