...
parser.add_argument('--capacity', type=int, default=None, help='Override link capacity (default: use config value)')
...
if args.capacity:
    ...
    print(f"Using specified link capacity: {effective_capacity}")
else:
    # Analyze traffic matrix to estimate required capacity
    if tm_index < len(tm_list):
        ...
        if avg_traffic_per_edge > link_capacity:
            ...
            effective_capacity = suggested_capacity
        else:
            effective_capacity = link_capacity
            print(f"Using original link capacity: {effective_capacity}")
    else:
        effective_capacity = link_capacity
        print(f"Using original link capacity: {effective_capacity}")
...
env = NetworkEnv(
    ...
    num_nodes=num_nodes,
    link_capacity=effective_capacity,  # Use adjusted capacity
    max_edges=max_edges,
    seed=seed
)
