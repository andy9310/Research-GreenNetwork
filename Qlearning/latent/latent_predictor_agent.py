...
class EnhancedQNetwork(nn.Module):
    """
    Q-Network that uses a latent state representation for action value prediction.
    
    This network takes the encoded state and predicts action values.
    Supports multiple architecture types:
    - 'mlp': Standard MLP architecture (default)
    - 'fatmlp': Larger MLP with increased capacity
    """
    
    def __init__(self, state_dim, action_dim, latent_dim=64, dropout_rate=0.2, architecture='mlp'):
        """
        ...
        if architecture == 'mlp':
            ...
            raise ValueError(f"Unknown architecture type: {architecture}. Use 'mlp' or 'fatmlp'")
    ...
    def forward(self, state):
        ...
        Returns:
            torch.Tensor: Action values of shape [batch_size, action_dim]
        """
        # Encode state into latent representation
        latent_state = self.state_encoder(state)
        
        # Forward through selected Q-network architecture
        return self.q_network(latent_state)
...
class LatentPredictorAgent:
    ...
    def __init__(self, state_dim, action_dim, latent_dim=64, predictor_hidden_dim=256,
                 ...
                 update_every=4, enable_predictor=True, architecture='mlp', device=None):
        """
        ...
        self.architecture = architecture
        self.qnetwork_local = EnhancedQNetwork(state_dim, action_dim, latent_dim, architecture=architecture).to(self.device)
        self.qnetwork_target = EnhancedQNetwork(state_dim, action_dim, latent_dim, architecture=architecture).to(self.device)
    ...
    def act(self, state, epsilon=0.0):
        ...
        if len(state.shape) == 1:
            ...
            # Batch of states already has batch dimension
            state_tensor = torch.from_numpy(state).float().to(self.device)
        
        # Print debug info for troubleshooting
        # print(f"DEBUG - State shape in act: {state_tensor.shape}")
