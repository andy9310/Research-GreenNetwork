...
class NetworkEnv(gym.Env):
    ...
    def __init__(self, adj_matrix, edge_list, tm_list, node_props, num_nodes, link_capacity, seed=None, max_edges=10, random_edge_order=False):
        ...
        # Observation: adjacency matrix representation (num_nodes x num_nodes) + current_edge_idx (1)
        # Each element [i][j] represents the usage of the edge from node i to node j (normalized by capacity)
        # For nodes with no direct connection, the value will be 0
        # Additionally, we include the current edge index being considered
        self.observation_space = spaces.Box(
            low=-np.inf, 
            high=np.inf, 
            shape=(self.num_nodes * self.num_nodes + 1,), 
            dtype=np.float32
        )
    ...
    def _get_observation(self):
        """Constructs an observation using adjacency matrix representation."""
        # Create adjacency matrix for link usage (normalized by capacity)
        usage_matrix = np.zeros((self.num_nodes, self.num_nodes), dtype=float)
        
        # Fill the adjacency matrix with normalized usage values
        for i in range(self.num_edges):
            if self.link_open[i] == 1:  # Only for open links
                u, v = self.edge_list[i]
                capacity = self.graph[u][v]['capacity']
                if capacity > 0:
                    # Normalized usage-to-capacity ratio [0-1+]
                    usage_ratio = self.usage[i] / capacity
                    # Since this is an undirected graph, set both directions
                    usage_matrix[u][v] = usage_ratio
                    usage_matrix[v][u] = usage_ratio  # Mirror for undirected graph
        
        # Flatten the matrix to a 1D array
        flattened_matrix = usage_matrix.flatten()
        
        # Include the current edge index (normalized)
        normalized_edge_idx = self.current_edge_idx / self.num_edges if self.num_edges > 0 else 0
        
        # Create observation vector: flattened adjacency matrix + current edge index
        obs = np.append(flattened_matrix, normalized_edge_idx).astype(np.float32)
    ...
    def step(self, action):
        ...
        # Check if we've made decisions for all edges
        if self.current_edge_idx >= self.num_edges:
            done = True
        
        # Add the real edge index to info (for debugging)
        info['real_edge_idx'] = edge_to_modify
        
        # Get updated observation and action mask
