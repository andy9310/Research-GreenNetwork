...
# Hyperparameters
# Use the hidden_dim from command line args (with default in argparse)
learning_rate = 5e-5  # Reduced learning rate for stability
gamma = 0.99
buffer_size = 100000 # Increased buffer size
batch_size = 256  # Larger batch size for better training og: 128
...
# Initialize the DQN agent with the selected architecture
if args.verbose:
    print(f"\nInitializing agent with {args.architecture} architecture and hidden_dim={args.hidden_dim}")
    
agent = DQN(state_dim, action_dim, hidden_dim=args.hidden_dim, lr=learning_rate, gamma=gamma, device=device, 
         network_type=args.architecture, nhead=4, num_layers=2)
...
if args.verbose:
    ...
    if args.architecture == 'fat_mlp':
        ...
        print(f"  Layer 3: {args.hidden_dim*4} neurons with LayerNorm and Dropout(0.3)")
        print(f"  Layer 4: {args.hidden_dim*2} neurons with LayerNorm and Dropout(0.2)")
        print(f"  Layer 5: {args.hidden_dim} neurons with LayerNorm and Dropout(0.1)")
        print(f"  Output: {action_dim} neurons (Q-values)")
        print(f"  Total parameters: Approximately {state_dim*args.hidden_dim + args.hidden_dim*args.hidden_dim*2 + args.hidden_dim*2*args.hidden_dim*4 + args.hidden_dim*4*args.hidden_dim*2 + args.hidden_dim*2*args.hidden_dim + args.hidden_dim*action_dim:,}")
...
# Include architecture in the model filename
model_path = f"models/dqn_{args.architecture}_{config_name}.pth"
if args.load_model:
...
for tm_idx, traffic_matrix in enumerate(training_tm_list):
    ...
    if args.verbose:
        # Some traffic matrices might be lists rather than numpy arrays, so check the type
        if hasattr(training_tm_list[tm_idx], 'shape'):
            print(f"Traffic matrix shape: {training_tm_list[tm_idx].shape}")
        else:
            print(f"Traffic matrix type: {type(training_tm_list[tm_idx]).__name__}")
    ...
    for episode in range(num_episodes_per_tm):
        state, _, _, _, _ = env.reset() # Env reset returns (obs, reward, done, truncated, info)
        episode_reward = 0
        done = False
        ...
        if (episode + 1) % print_interval == 0:
            ...
            if args.verbose and (episode + 1) % (print_interval * 5) == 0:
                # Print sample Q-values to monitor learning progress
                with torch.no_grad():
                    sample_state = replay_buffer.memory[-1].state if len(replay_buffer) > 0 else env._get_observation()
                    sample_state = torch.FloatTensor(sample_state).to(device)
                    q_vals = agent.qnetwork_local(sample_state)
                    tm_progress.write(f"  Sample Q-values: close={q_vals[0]:.3f}, open={q_vals[1]:.3f}, diff={q_vals[1]-q_vals[0]:.3f}")
...
config_name = config_file.split('.')[0]  # Remove .json extension
# Include architecture in the filename to distinguish between different models
model_save_path = f"models/dqn_{args.architecture}_{config_name}.pth"
